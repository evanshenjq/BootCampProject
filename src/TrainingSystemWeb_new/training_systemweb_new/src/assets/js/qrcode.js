export default {
  ...
  methods:{
    getUrl(event) {
      ...
      var self=this;
      this.getUrls(event.target, function(url1, url2) {
        //url1=info,url2=img
        //error qrcode
        if(url1=="error decoding QR Code"){
          $('#error_info').text("Please upload qrcode again!");
          $('#error_info').show();
        }
        $("#img").attr('src',url2);
        if(!self.isJsonString(url1)){
          $('#error_info').text("Please upload right qrcode!");
          $('#error_info').show();
        }else{
          self.fetch_right_qr(url1);
        }
      });

    },
    fetch_right_qr(url1){
      var courseInfo=JSON.parse(url1);
      console.log(courseInfo);
      if(courseInfo.courseId==undefined || courseInfo.courseTime==undefined){
        $('#error_info').text("Please upload right qrcode!");
        $('#error_info').show();
      }else{
        //success get qr code!
        ...
        formdata.append('userId',$.session.get('userId'));
        formdata.append('courseId',courseInfo.courseId);
        formdata.append('courseTime',courseInfo.courseTime);
        console.log(courseInfo.courseId);
        console.log(courseInfo.courseTime);
        var url="/perficient/signInCourse";
        fetch(url,{
          method:'POST',
          body:formdata
    ...
    close(){
      $('#error_info').hide();
      $('img').attr('src',"http://10.2.1.123:8080/perficient/wahaha/qrcode.png");
    },
    //util function
    getUrls(elem, fn) {
      let url = null,
